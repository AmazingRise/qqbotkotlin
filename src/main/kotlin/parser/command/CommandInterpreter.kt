                ...
                try {
                    ...
                    if (sentence=="")
                    {
                        ...
                        return CommandResult(
                            "Reminder of ${args[1].toLong()} has been removed.",
                            ErrorLevel.FAILED
                        )
                    }else{
                        Global.reminders.put(args[1].toLong(),sentence)
                        return CommandResult(
                            "Done. (Reminder of ${args[1].toLong()})",
                            ErrorLevel.FAILED
                        )
                    }
                } catch (e: Exception) {
    ...
    fun parseSuperCommand(superCommand: String): CommandResult {
        ...
        try {
            when (args[0]) {
                ...
                "stop_server" -> {
                    Prompt.echo("Shutdown signal received.",PromptLogType.OK)
                    if (!Archive.saveKeywordsToFile(File(Global.defaultArchiveLocation))) {
                        Prompt.echo("Error saving configuration file.", PromptLogType.FAILED)
                    }
                    network.Server.stop()
                    exitProcess(0)
                }
                ...
                "addop" -> {
                    try {
                        ...
                        return CommandResult(
                            "Operator ${args[1]} has been added.",
                            ErrorLevel.SUCCESS
                        )
                ...
                "delop" -> {
                    try {
                        ...
                        return CommandResult(
                            "Operator ${args[1]} has been removed.",
                            ErrorLevel.SUCCESS
                        )
                    } catch (e: NumberFormatException) {
                        return CommandResult("Id invalid.", ErrorLevel.FAILED)
                ...
                "unban" -> {
                    try {
                        ...
                        return CommandResult(
                            "${args[1]} has been removed from blacklist.",
                            ErrorLevel.SUCCESS
                        )
                    } catch (e: NumberFormatException) {
                        return CommandResult("Id invalid.", ErrorLevel.FAILED)
